...
export const CustomToastView: React.FC<CustomToastViewProps> = ({ toast, onDismiss, children }) => {
  ...
  useEffect(() => {
    ...
    // Auto dismiss timer
    const duration =
      typeof toast.duration === 'number' ? toast.duration : toast.duration === 'long' ? 3500 : 2000
  ...
  const getPositionStyle = () => {
    switch (toast.position) {
      case 'top':
        return { top: 50 }
      case 'center':
        return { top: '50%' as const, marginTop: -50 }
      default:
        return { bottom: 50 }
    }
  }
  ...
  if (toast.customView) {
    return (
      ...
      >
        ...
        >
          {typeof toast.customView === 'function'
            ? toast.customView({ toast, onDismiss: animateOut })
            : toast.customView}
        </TouchableOpacity>
      </Animated.View>
    )
  ...
  return (
    ...
    >
      ...
      >
        {children || (
          <View style={styles.defaultContent}>
            {toast.icon && typeof toast.icon === 'string' && (
              <Text style={[styles.icon, { color: toast.textColor || '#fff' }]}>{toast.icon}</Text>
            )}
            <Text style={[styles.message, { color: toast.textColor || '#fff' }]} numberOfLines={3}>
              {toast.message}
            </Text>
            {(toast.actions || (toast.action ? [toast.action] : [])).map((action, index) => (
              <TouchableOpacity
                key={`action-${index}`}
                onPress={() => {
                  action.onPress()
        ...
        {toast.showProgressBar && toast.progress !== undefined && (
