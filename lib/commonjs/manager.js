...
class ToastManager {
  ...
  constructor() {
    this.queue = new _queue.ToastQueue({
  ...
  async initializePersistence() {
    if (this.config.persistenceEnabled && _reactNative.Platform.OS !== 'web') {
      try {
        await _persistence.toastPersistence.enable();
        const persistedToasts = await _persistence.toastPersistence.load();
        ...
        // Start auto-save
        _persistence.toastPersistence.startAutoSave(() => [...this.queue.getQueue(), ...this.queue.getAllActive()], this.config.persistenceInterval || 1000);
  ...
  configure(config) {
    ...
    if (config.analyticsEnabled !== undefined) {
      _analytics.toastAnalytics.setEnabled(config.analyticsEnabled);
    }
    ...
    if (config.persistenceEnabled !== undefined || config.persistenceInterval !== undefined) {
      if (this.config.persistenceEnabled && _reactNative.Platform.OS !== 'web') {
        if (!prevPersistenceEnabled) {
          ...
          _persistence.toastPersistence.stopAutoSave();
          _persistence.toastPersistence.startAutoSave(() => [...this.queue.getQueue(), ...this.queue.getAllActive()], config.persistenceInterval);
        }
      } else {
        _persistence.toastPersistence.disable();
      }
    }
  ...
  show(options) {
    ...
    if (this.config.analyticsEnabled) {
      _analytics.toastAnalytics.trackToastQueued(queuedToast);
    }
    ...
    if (_reactNative.Platform.OS !== 'web') {
      ...
      if (hapticType) {
        (0, _utils.triggerHaptic)(hapticType);
      }
    }
    ...
    if (queuedToast.customView) {
      ...
      const duration = (0, _utils.calculateDuration)(queuedToast.duration);
  ...
  async showNativeToast(toast, isRetry = false) {
    try {
      ...
      if (this.config.analyticsEnabled) {
        _analytics.toastAnalytics.trackToastShown(toast);
      }
      ...
      if (attempts < this.maxRetries) {
        ...
        if (this.config.analyticsEnabled) {
          _analytics.toastAnalytics.trackToastError(error, toast);
        }
      }
    }
  }
  ...
  hideToast(id) {
    ...
    if (this.config.analyticsEnabled) {
      _analytics.toastAnalytics.trackToastHidden(toast, 'manual');
    }
  ...
  update(id, options) {
    ...
    if (options.duration !== undefined) {
      this.clearTimer(id);
      const duration = (0, _utils.calculateDuration)(options.duration);
      const timer = setTimeout(() => {
        this.hideToast(id);
      }, duration);
